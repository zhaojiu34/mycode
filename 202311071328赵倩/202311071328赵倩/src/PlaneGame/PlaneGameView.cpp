...
void CPlaneGameView::UpdateFrame(CDC* pMemDC)
{
	...
	if (level == 1)
	{
		...
		CBitmap m_bmp;
		m_bmp.LoadBitmapW(IDB_BACKGROUNAD1);//创建位图
	...
	if (level == 2)
	{
		...
		m_CDC.CreateCompatibleDC(pMemDC);
		CBitmap m_bmp;
		m_bmp.LoadBitmapW(IDB_BACKAGROUND2);//创建位图
		CBitmap* pbmpold = m_CDC.SelectObject(&m_bmp);
		pMemDC->BitBlt(0, ImageLocation, GAME_WIDTH, GAME_HEIGHT - ImageLocation, &m_CDC, 0, 0, SRCCOPY);//设置动态背景
		pMemDC->BitBlt(0, 0, GAME_WIDTH, ImageLocation, &m_CDC, 0, GAME_HEIGHT - ImageLocation, SRCCOPY);
		m_CDC.SelectObject(pbmpold);
	...
	if (level == 3)
	{
		...
		m_CDC.CreateCompatibleDC(pMemDC);
		CBitmap m_bmp;
		m_bmp.LoadBitmapW(IDB_BACKGROUND3);//创建位图
		CBitmap* pbmpold = m_CDC.SelectObject(&m_bmp);
		pMemDC->BitBlt(0, ImageLocation, GAME_WIDTH, GAME_HEIGHT - ImageLocation, &m_CDC, 0, 0, SRCCOPY);//设置动态背景
		pMemDC->BitBlt(0, 0, GAME_WIDTH, ImageLocation, &m_CDC, 0, GAME_HEIGHT - ImageLocation, SRCCOPY);
		m_CDC.SelectObject(pbmpold);
...
void CPlaneGameView::AI()
{
	...
	//随机产生敌机1
	////控制敌机1的生成时机，并在一个较短的时间间隔内控制敌人的生成频率
	...
	if (GetKey(VK_CONTROL) == 1)//按下了ctrl键：多发子弹
	{
		blue -= 0.5;
		if (blue > 0)
		{
	...
	{
		CBall* pBall = (CBall*)m_ObjList[enBall].GetNext(bPos1);//将bPos1移动到下一个位置
		CRect bRect = pBall->GetRect();
		CRect tmpRect;

		if (tmpRect.IntersectRect(&bRect, mRect))//判断敌机和子弹是否有重叠
		{
			//添加爆炸效果
	...
	{
		KillTimer(1);
		if (AfxMessageBox(L"恭喜！全部通过啦~，还要再来一次吗？", MB_YESNO) == 6)
		{
			for (int i = 0; i < 11; i++)
				m_ObjList[i].RemoveAll();
			delete m_pMe;
